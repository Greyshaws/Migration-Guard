...
contract MonitorOnce is Script {
    ...
    function run() external {
        ...
        address[] memory approved = _parseAddressCsv("UG_APPROVED_OWNERS_CSV");
        uint256 failures = 0;

        console.log("");
        console.log("Target:");
        console.log("  ", adminAddr);
        console.log("Live block:", block.number);
        console.log("");
        console.log("-------------------------------------------");
        ...
        if (ownerEnabled) {
            ...
            if (!r.ok) { _emitAlert("auth.OwnerCheck", r); failures++; }
        }

        if (originEnabled) {
            PropertyResult memory r = originCheck.check(adminAddr);
            _printResult("auth.DetectTxOrigin", r);
            if (!r.ok) { _emitAlert("auth.DetectTxOrigin", r); failures++; }
        }
        ...
        if (oracleEnabled) {
            ...
            if (!r.ok) { _emitAlert("scaling.MatchOracleDecimals", r); failures++; }
        }
        ...
        if (storageEnabled) {
            ...
            if (!r.ok) { _emitAlert("storage.ProxyStateCheck", r); failures++; }
        }
        ...
        if (deadEnabled) {
            ...
            if (!r.ok) { _emitAlert("surface.FindLiveSelectors", r); failures++; }
        }
        ...
        if (liqEnabled) {
            ...
            if (!r.ok) { _emitAlert("liquidity.PoolDepthCheck", r); failures++; }
        }

        console.log("===========================================");
        if (failures == 0) {
            console.log("Monitor: clean (no alerts)");
        } else {
            console.log("Monitor: ALERTING");
            console.log("Active alerts:", failures);
        }
    }

    function _emitAlert(string memory propertyId, PropertyResult memory r) internal view {
        console.log("");
        console.log(">>> ALERT >>>");
        ...
        console.log("  (In production: webhook fires here)");
        console.log(">>>>>>>>>>>>>");
    }
