...
upgrade-guard/
├── bin/                              CLI wrappers (bash + python)
│   ├── upgrade-guard                 Pre-deploy / CI runner
│   ├── upgrade-guard-monitor         Post-deploy live monitor
│   ├── upgrade-guard-init            Starter config generator
│   └── parse-config.py               TOML → environment variable parser
├── src/
│   ├── IMigrationProperty.sol        Shared property interface
│   └── properties/                   Six property implementations
├── script/
│   ├── RunAll.s.sol                  CI / ad-hoc runner (Foundry script)
│   └── MonitorOnce.s.sol             Live monitor runner (Foundry script)
├── test/                             33 Foundry tests
├── .github/workflows/                CI workflow
├── upgrade-guard.toml                Shipped Aevo config
